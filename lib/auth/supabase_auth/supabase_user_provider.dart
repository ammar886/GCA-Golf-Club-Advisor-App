...
class GolfClubAdvisorSupabaseUser extends BaseAuthUser {
  GolfClubAdvisorSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> golfClubAdvisorSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GolfClubAdvisorSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
